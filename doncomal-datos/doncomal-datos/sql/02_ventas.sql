...
-- Las secciones del menú. Viven en su propia tabla, y no como una lista fija
-- dentro de una restricción, para que Octavio pueda agregar o quitar una desde
-- la app sin tocar SQL. 'orden' es el lugar en que salen en pantalla.
create table if not exists dc_cat_categorias (
...
-- Dos clasificaciones que no son la misma y no se estorban:
--
--   familia    La FORMA del producto: Gordita, Bocol, Taco, Enchilada, Huevos.
--              La pone el parser al importar y sirve para agrupar lo que se
--              cocina igual.
--   categoria  La SECCIÓN DEL MENÚ: Clásicos, Antojitos, Platillos, Postres,
--              Bebidas, Extras. La pone Octavio y sirve para leer el negocio
--              como lo lee él.
--
-- Una media orden de enchiladas y una orden completa son la misma familia y la
-- misma categoría; unas Gorditas de Azúcar son familia Gordita pero categoría
-- Postres. Por eso hacen falta las dos.
create table if not exists dc_cat_productos (
  producto         text not null,
  familia          text not null default 'Otros',
  categoria        text references dc_cat_categorias(categoria)
  ...
  -- Cuántos guisados trae el precio. Casi todo trae uno: una gordita con dos
  -- guisos paga el segundo. La migada es la excepción — su precio ya incluye
  -- dos—, y es una regla de negocio, no algo que se adivine de los datos.
  guisados_incluidos int not null default 1 check (guisados_incluidos > 0),
  lleva_guisado    boolean not null default false,
  activo           boolean not null default true,
  primary key (producto)
);

-- Para las instalaciones que ya existían antes de que hubiera categorías:
-- 'create table if not exists' no agrega columnas a una tabla que ya está.
alter table dc_cat_productos add column if not exists categoria text;
alter table dc_cat_productos
  add column if not exists guisados_incluidos int not null default 1;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'dc_cat_productos_categoria_fkey') then
    alter table dc_cat_productos
      add constraint dc_cat_productos_categoria_fkey
      foreign key (categoria) references dc_cat_categorias(categoria)
      on update cascade on delete set null;
  end if;
end $$;

-- 'grupo' junta varios modificadores que para el negocio son el mismo: los
-- cuatro términos de cocción del huevo estrellado —medio, tierno, cocido,
-- ciego— cuestan lo mismo y se piden por lo mismo. Separados, cada uno parece
-- un extra menor; juntos son uno de los que más se piden. La app enseña el
-- grupo y deja abrirlo para ver los términos.
...
-- Para las instalaciones que ya existían: 'if not exists' no agrega columnas.
alter table dc_cat_modificadores add column if not exists grupo text;
...
create table if not exists dc_ventas_detalle (
  -- "by default" y no "always": la carga inicial trae los id ya asignados
  -- porque dc_ventas_modificadores tiene que apuntar a ellos. Después de la
  -- carga se reinicia la secuencia (ver el final de este archivo).
...
begin
  foreach t in array array[
    'dc_cat_productos','dc_cat_modificadores','dc_cat_categorias',
    'dc_importaciones',
...
grant select, insert, update, delete on
  dc_cat_productos, dc_cat_modificadores, dc_cat_categorias, dc_importaciones,
  dc_ventas_dia, dc_ventas_detalle, dc_ventas_modificadores
to authenticated;
...
-- -----------------------------------------------------------------------------
-- Después de la carga inicial por CSV: reiniciar las secuencias
--
-- Al importar con los id ya puestos, el contador interno se queda en 1 y el
-- siguiente insert chocaría con un id existente. Esto lo pone al día.
-- Correr UNA vez, después de cargar el histórico.
-- -----------------------------------------------------------------------------

select setval(pg_get_serial_sequence('dc_ventas_detalle','id'),
              coalesce((select max(id) from dc_ventas_detalle), 0) + 1, false);
select setval(pg_get_serial_sequence('dc_ventas_modificadores','id'),
              coalesce((select max(id) from dc_ventas_modificadores), 0) + 1, false);
select setval(pg_get_serial_sequence('dc_importaciones','id'),
              coalesce((select max(id) from dc_importaciones), 0) + 1, false);
