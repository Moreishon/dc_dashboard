...
import Catalogo from '../src/pantallas/Catalogo.jsx';
...
function Banco() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [dias, guisadoMes, prodDia, guisDia, modDia, categorias] = await Promise.all([
        traer('dias'), traer('guisado_mes'),
        traer('producto_dia'), traer('guisado_dia'), traer('modificador_dia'),
        traer('categorias'),
      ]);
      const [extraDia, extraPrecio] = await Promise.all([
        traer('extra_dia'), traer('extra_precio'),
      ]);
      // Los agregadores falsos leen de aquí.
      window.__falso = { prodDia, guisDia, modDia, categorias,
                         extraDia, extraPrecio };
    ...
    <>
      ...
      <div style={pagina(esAncho)}>
        <div style={{ display: 'flex', gap: 2, background: C.negro, padding: '0 12px' }}>
          {['resumen', 'productos', 'guisados', 'catalogo'].map((p) => (
        ...
        {pest === 'catalogo' && <Catalogo perfil={{ nombre: 'Octavio', rol: 'admin' }} />}
      </div>
    </>
  );
}
