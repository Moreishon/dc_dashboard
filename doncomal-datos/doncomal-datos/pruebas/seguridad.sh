...
for f in 01_seguridad 02_ventas 03_importacion 04_por_rango 05_permisos \
         06_cobertura_rapida 07_agregados 08_rangos 09_cerrar_funciones \
         10_categorias 11_extras; do
...
# 6 · reclasificar un producto es escribir en el catálogo, no consultar
psql -q -d "$BASE" -c "update dc_usuarios set rol='lectura' where user_id='$UID_ADMIN';" >/dev/null 2>&1
s=$(corre authenticated "$UID_ADMIN" "select dc_guardar_categoria('Gorditas','Postres');")
case "$s" in *"no puede hacer esto"*) ok "rol de lectura: no puede reclasificar";;
  *) mal "el rol de lectura cambió una categoría → $s";; esac
psql -q -d "$BASE" -c "update dc_usuarios set rol='admin' where user_id='$UID_ADMIN';" >/dev/null 2>&1

# 7 · una categoría inventada no debe entrar: crearía una sección fantasma en
#     el tablero, con productos adentro y sin ningún error a la vista.
s=$(corre authenticated "$UID_ADMIN" "select dc_guardar_categoria('Gorditas','Postre');")
case "$s" in *'no existe'*) ok "una categoría mal escrita se rechaza";;
  *) mal "entró una categoría que no existe → $s";; esac

# 8 · cambiar los guisados incluidos es una regla de negocio, no una consulta
psql -q -d "$BASE" -c "update dc_usuarios set rol='lectura' where user_id='$UID_ADMIN';" >/dev/null 2>&1
s=$(corre authenticated "$UID_ADMIN" "select dc_guardar_guisados('Gorditas', 2);")
case "$s" in *"no puede hacer esto"*) ok "rol de lectura: no puede cambiar los guisados incluidos";;
  *) mal "el rol de lectura cambió una regla de negocio → $s";; esac
psql -q -d "$BASE" -c "update dc_usuarios set rol='admin' where user_id='$UID_ADMIN';" >/dev/null 2>&1

# 9 · cero guisados incluidos dejaría el cálculo de extras sin sentido
s=$(corre authenticated "$UID_ADMIN" "select dc_guardar_guisados('Gorditas', 0);")
case "$s" in *"van de 1 a 9"*) ok "cero guisados incluidos se rechaza";;
  *) mal "aceptó cero guisados incluidos → $s";; esac

echo
echo "Lectura anónima"
for objeto in dc_ventas_detalle dc_ventas_dia dc_v_dias dc_v_cobertura \
              dc_v_catalogo dc_v_extras_detalle dc_v_t_extras_mes \
              dc_cat_categorias dc_cat_productos dc_usuarios app_data; do
