...
begin
  ...
  -- Las de extras van al final: dc_m_extras_mes lee de dc_m_precios y de
  -- dc_m_extra_producto, así que las dos tienen que estar frescas antes.
  -- Se comprueba que existan porque 11 puede no haberse corrido todavía.
  if to_regclass('public.dc_m_extra_producto') is not null then
    refresh materialized view dc_m_extra_producto;
  end if;
  if to_regclass('public.dc_m_extra_precio') is not null then
    refresh materialized view dc_m_extra_precio;
  end if;
  if to_regclass('public.dc_m_extra_precio_gral') is not null then
    refresh materialized view dc_m_extra_precio_gral;
  end if;
  -- Esta lee de las dos anteriores, así que va después.
  if to_regclass('public.dc_m_extra_precio_usado') is not null then
    refresh materialized view dc_m_extra_precio_usado;
  end if;
  if to_regclass('public.dc_m_extras_mes') is not null then
    refresh materialized view dc_m_extras_mes;
  end if;
end;
$$;
...
-- futura se quede fuera por olvido.
--
-- Va empaquetado en una función y no suelto en un bloque, por una razón muy
-- concreta: una función recién creada en Supabase nace con EXECUTE para el
-- anónimo. Cualquier archivo que agregue funciones tiene que cerrarlas él
-- mismo, ahí mismo, y no confiar en que alguien se acuerde de volver a correr
-- este. Con esto, ese archivo termina con 'select dc_cerrar_funciones();' y ya.
--
-- NO es security definer, a propósito. Es lo único que reparte permisos en todo
-- el esquema, y corre con los del que la llama: desde el SQL Editor —que es el
-- dueño de la base— funciona, y desde el navegador no puede hacer nada aunque
-- alguien la llamara. Una función que ejecuta GRANT y corre con permisos
-- elevados sería el agujero más grande del proyecto.
--
-- La lista de abajo es la única puerta de entrada de la app a la base. Si
-- agregas una función que el tablero va a llamar, su nombre va aquí; si se te
-- olvida, la app falla con "permission denied" — ruidoso, que es como debe
-- fallar.
-- =============================================================================

create or replace function dc_cerrar_funciones()
returns int
language plpgsql
as $$
declare
...
begin
  ...
  loop
    ...
    if f.proname in ('dc_mi_rol', 'dc_es', 'dc_exigir', 'dc_importar_mes',
                     ...
                     'dc_guisados_rango', 'dc_modificadores_rango',
                     'dc_guardar_categoria', 'dc_extras_rango',
                     'dc_guardar_guisados') then
...
-- Que nadie con sesión pueda llamarla: no la necesita y no le sirve.
revoke all on function dc_cerrar_funciones() from anon, authenticated, public;

select dc_cerrar_funciones() as funciones_cerradas;
