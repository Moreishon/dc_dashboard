...
// ─── 1b · la misma pantalla en un monitor ────────────────────────────────────
{
  ...
  // El formulario no debe estirarse de lado a lado en un monitor: se queda
  // en una columna legible.
  const anchoForm = await pag.locator('form').evaluate((e) => e.offsetWidth);
  ok(anchoForm <= 420, `el formulario mide ${anchoForm}px, no se estira`);

  ok(errores.length === 0,
     `sin errores de JavaScript${errores.length ? ': ' + errores[0] : ''}`);

  await pag.screenshot({ path: '/tmp/dc_escritorio.png' });
  await pag.close();
}
